(function () {
  ...
  var KEY = document.body.getAttribute("data-line-key") || "";
  var LINE = CONFIG[KEY] || {};
  var API_BASE = (CONFIG.API_BASE || "").replace(/\/$/, "");
  var GROUPS = LINE.groups || [];
  var PAGE = LINE.href || (KEY.toLowerCase().replace(/_line$/, "") + ".html");
  var FALLBACK = LINE.fallback || [];
  var LINE_IDS = [];
  ...
  function shortLabel(p) {
    return productMeta(p.id).kicker || p.name || "";
  }
